...
import { getAuth, onAuthStateChanged, signInWithEmailAndPassword, createUserWithEmailAndPassword, sendPasswordResetEmail, updatePassword } from 'https://www.gstatic.com/firebasejs/9.23.0/firebase-auth-compat.js';
import { getFirestore, doc, getDoc, setDoc, updateDoc, collection, addDoc, query, getDocs, orderBy, where } from 'https://www.gstatic.com/firebasejs/9.23.0/firebase-firestore-compat.js';
...
export async function login() {
    ...
    try {
        ...
        // Cek apakah akun bawaan belum dibuat
        const DEFAULT_ROLES = {
            ...
            try {
                ...
                if (docSnap.exists()) {
                    ...
                    if (data.isFirstLogin) {
                        // Tampilkan modal force change password
...
async function showForceChangePasswordModal(uid) {
    const newPass = prompt('⚠️ Anda harus mengganti password sebelum masuk. Masukkan password baru:');
...
// Enkripsi sederhana (pindahkan ke utils.js nanti)
...
function getCurrentAllowedRoles() {
    if (currentUserRole === 'ketua') return ['ketua', 'bendahara', 'sekretaris', 'acara', 'logistik', 'akademi', 'dekdok', 'humas', 'dpl'];
...
// Render page berdasarkan role (memuat modul role yang sesuai)
